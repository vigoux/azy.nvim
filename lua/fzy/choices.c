  ...
  for (;;) {
    ...
    for (size_t i = start; i < end; i++) {
      if (has_match(job->choices->prompt, c->strings[i])) {
        ...
        result->list[result->size].score = match(job->choices->prompt, c->strings[i]);
        result->size++;
      }
    }
  }
...
struct result_list run_search(choices_t *c, size_t start, size_t end)
{
...
void choices_add_incremental(choices_t *c, const char *choices[], size_t n) {
  ...
  if (c->available && c->prompt[0] != '\0') {
    // Now we need to sort the entries we added, then merge them with the rest of
    // the entries
