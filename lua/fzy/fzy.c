...
static choices_ud_t *
fzy_choices_check(lua_State *L, int index)
{
	return (choices_ud_t *)luaL_checkudata(L, index, CHOICES_META);
}

static int
fzy_choices_gc(lua_State *L)
{
...
static int
fzy_choices_create(lua_State *L)
{
...
static int
fzy_choices_populate(lua_State *L)
{
...
static int
fzy_choices_populate_incremental(lua_State *L)
{
	choices_ud_t * c = fzy_choices_check(L, 1);
	luaL_checktype(L, 2, LUA_TTABLE);
...
static int
fzy_choices_available(lua_State *L)
{
	choices_ud_t * c = fzy_choices_check(L, 1);
...
static int
fzy_choices_search(lua_State *L)
{
	choices_ud_t * c = fzy_choices_check(L, 1);
...
static int
fzy_choices_elements(lua_State *L)
{
	choices_ud_t * c = fzy_choices_check(L, 1);
...
static int
push_item(lua_State *L, choices_t *c, size_t index)
{
...
static int
push_selected(lua_State *L, choices_t *c)
{
	return push_item(L, c, c->selection);
}

static int
fzy_choices_selected(lua_State *L)
{
	choices_ud_t * c = fzy_choices_check(L, 1);
